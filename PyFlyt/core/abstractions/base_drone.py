...
class DroneClass(ABC):
    ...
    def __init__(
        ...
        if color == "red":
            self.drone_path = os.path.join(model_dir, f"{drone_model}/{drone_model}_red.urdf")
        elif color == "green":
            self.drone_path = os.path.join(model_dir, f"{drone_model}/{drone_model}_green.urdf")
        else:
